...
#include "Asset.h"
#include "Artifacts/ArtifactLease.h"
#include "Artifacts/ResolvedArtifact.h"
#include "Engine/Core/Types/Pair.h"
#include "Storage/FlaxStorageReference.h"

/// <summary>Result of a transactional binary asset storage switch.</summary>
enum class BinaryAssetStorageSwitchResult : byte
{
    Success,
    InvalidThread,
    InvalidArtifact,
...
API_CLASS(Abstract, NoSpawn) class FLAXENGINE_API BinaryAsset : public Asset
{
    ...
    friend Content;
    friend class LoadAssetDataTask;
protected:
    AssetHeader _header;
    ...
    bool _isSaving;
    bool _isUsingExactArtifact;
    bool _isGeneratedArtifact;
    ArtifactLoadDisposition _artifactLoadDisposition;
...
public:
    ...
    /// <summary>Attaches resolved artifact metadata before storage initialization.</summary>
    void SetResolvedArtifact(const ResolvedArtifact& artifact);

    /// <summary>
    /// Atomically transitions a loaded asset to validated immutable storage and rolls back on load failure.
    /// Must be called from the main thread.
    /// </summary>
    BinaryAssetStorageSwitchResult SwitchStorage(const ResolvedArtifact& artifact);

protected:
    /// <summary>Called after replacement storage is validated but before the resource reload begins.</summary>
    virtual void OnBeforeArtifactStorageChange()
    {
    }

    /// <summary>Called after the resource has loaded successfully from replacement storage.</summary>
    virtual void OnAfterArtifactStorageChange()
    {
    }
...
public:
    /// <summary>Gets the physical storage used by this binary asset.</summary>
    API_PROPERTY() StringView GetStoragePath() const;

    /// <summary>Gets the resolved artifact key, or empty for legacy storage.</summary>
    API_PROPERTY() StringView GetArtifactKey() const
    {
        return _artifactKey;
    }

    /// <summary>Returns true when the selected artifact exactly matches the current request.</summary>
    API_PROPERTY() bool IsUsingExactArtifact() const
    {
        return _isUsingExactArtifact;
    }

    /// <summary>Returns true when physical storage is a generated Library artifact.</summary>
    bool IsUsingGeneratedArtifact() const
    {
        return _isGeneratedArtifact;
    }

    /// <summary>Returns true while generated storage is protected from artifact-store cleanup.</summary>
    bool HasArtifactLease() const
    {
        return _artifactLease.IsValid();
    }

    /// <summary>Returns true when selected generated bytes must be rebuilt instead of upgraded in place.</summary>
    bool NeedsArtifactRebuild() const
    {
        return _artifactLoadDisposition == ArtifactLoadDisposition::RebuildRequired;
    }

    /// <summary>Marks this load as requiring generated artifact regeneration.</summary>
    void MarkArtifactRebuildRequired()
    {
        _artifactLoadDisposition = ArtifactLoadDisposition::RebuildRequired;
    }
...
#if USE_EDITOR
    bool canAutoSaveDeprecatedData() const final override
    {
        return !_isGeneratedArtifact;
    }
#endif
