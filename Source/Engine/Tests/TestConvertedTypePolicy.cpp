...
TEST_CASE("Converted type lockout forbids new flax authoring and keeps legacy exceptions")
{
    ...
    CHECK(ConvertedTypePolicy::AllowsLegacyBinaryAuthoring(settings, TEXT("FlaxEngine.Material"), TEXT("Content/M.flax")));
    CHECK(ConvertedTypePolicy::IsConvertedGraphType(TEXT("FlaxEngine.VisualScript")));
    CHECK(ConvertedTypePolicy::IsLegacyExceptionType(TEXT("FlaxEngine.Scene")));
    CHECK_FALSE(ConvertedTypePolicy::IsConvertedGraphType(TEXT("FlaxEngine.Scene")));
