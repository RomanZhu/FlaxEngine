...
TEST_CASE("Source asset database restores affected rows after rollback and failed validation")
{
    ...
    Array<SourceAssetObjectRow> objects;
    objects.Add(MakeObject(firstId, 1, TEXT("main")));
    objects.Add(MakeObject(firstId, 2, TEXT("sub")));
    seed->ReplaceObjects(firstId, objects);
    SourceAssetDependencyRow dependency;
    dependency.OwnerAssetGuid = firstId;
    ...
    dependency.TargetAssetGuid = firstId;
    ...
    seed->ReplaceDependencies(firstId, 1, dependency.TargetId, dependencies);
    ...
    dependencies.Clear();
    rolledBack->ReplaceDependencies(firstId, 0, dependency.TargetId, dependencies);
    rolledBack->Rollback();
