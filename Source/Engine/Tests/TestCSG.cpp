...
#include "Engine/CSG/CSGGeneratedOutputPaths.h"
#include "Engine/Level/Actors/CSGModel.h"
#include "Engine/Level/Actors/CSGStack.h"
#include "Engine/Level/Actors/BoxBrush.h"
#include "Engine/CSG/CSGCompilation.h"

TEST_CASE("CSGCompiledData transient preview and surface queries")
{
    CSG::CSGCompiledData csgData;
    CHECK_FALSE(csgData.HasPreviewOverride());
    CHECK_FALSE(csgData.HasPersistedData());
    CHECK_FALSE(csgData.HasRenderableData());

    // Publish transient surface metadata
    CSG::RawData rawData;
    Guid brushId = Guid::New();
    auto& brushEntry = rawData.Brushes[brushId];
    brushEntry.Surfaces.Resize(1);
    CSG::RawData::SurfaceTriangle tri;
    tri.V[0] = Float3(0, 0, 0);
    tri.V[1] = Float3(10, 0, 0);
    tri.V[2] = Float3(0, 10, 0);
    brushEntry.Surfaces[0].Triangles.Add(tri);

    csgData.PublishPreviewSurfaceData(rawData);
    csgData.MarkPreviewOverrideActive();
    CHECK(csgData.HasPreviewOverride());

    CSG::CSGCompiledData::SurfaceData surfaceData;
    CHECK(csgData.TryGetSurfaceData(brushId, 0, surfaceData));
    CHECK(surfaceData.Triangles.Count() == 1);
    CHECK_FALSE(csgData.TryGetSurfaceData(Guid::New(), 0, surfaceData));
    CHECK_FALSE(csgData.TryGetSurfaceData(brushId, 1, surfaceData));

    // Publish empty preview
    csgData.PublishEmptyPreview();
    CHECK(csgData.HasPreviewOverride());
    CHECK_FALSE(csgData.TryGetSurfaceData(brushId, 0, surfaceData));
    CHECK_FALSE(csgData.HasRenderableData());

    // Clear transient preview
    csgData.ClearTransientPreview();
    CHECK_FALSE(csgData.HasPreviewOverride());
}

TEST_CASE("CSG output path resolver for assets")
{
    Guid prefabId = Guid::New();
    auto model = New<CSGModel>(ScriptingObject::SpawnParams(Guid::New(), CSGModel::TypeInitializer));
    CSG::CSGGeneratedOutputPaths paths;

    REQUIRE(CSG::CSGGeneratedOutputPathResolver::ResolveForAsset(prefabId, model, paths));
    CHECK(paths.Model.Contains(TEXT("PrefabData")));
    CHECK(paths.Model.Contains(prefabId.ToString(Guid::FormatType::N)));
    CHECK(paths.RawData.Contains(TEXT("PrefabData")));
    CHECK(paths.Collision.Contains(TEXT("PrefabData")));

    Delete(model);
}

TEST_CASE("CSGModel compiles without Scene")
{
    auto model = New<CSGModel>(ScriptingObject::SpawnParams(Guid::New(), CSGModel::TypeInitializer));
    auto stack = New<CSGStack>(ScriptingObject::SpawnParams(Guid::New(), CSGStack::TypeInitializer));
    auto brush1 = New<BoxBrush>(ScriptingObject::SpawnParams(Guid::New(), BoxBrush::TypeInitializer));
    auto brush2 = New<BoxBrush>(ScriptingObject::SpawnParams(Guid::New(), BoxBrush::TypeInitializer));
    ...
    stack->SetParent(model, false, false);

    CSG::Mesh combinedMesh;
    REQUIRE(CSG::CSGCompilation::CompileTargetMeshes(model, combinedMesh));
    CHECK(combinedMesh.GetPolygons()->HasItems());

    CSG::RawData rawData;
    Array<CSG::MeshVertex> vertexBuffer;
    combinedMesh.Triangulate(rawData, vertexBuffer);
    rawData.RemoveEmptySlots();
    CHECK(rawData.Slots.HasItems());
