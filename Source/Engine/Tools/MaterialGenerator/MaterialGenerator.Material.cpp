...
void MaterialGenerator::ProcessGroupMaterial(Box* box, Node* node, Value& value)
{
    ...
    {
        // Load function asset
        const auto function = Assets.Load<MaterialFunction>(Content::ResolveRuntimeObjectId((Guid)node->Values[0]));
        if (!function)
        {
        ...
        for (int32 i = _callStack.Count() - 1; i >= 0; i--)
        {
            ...
            {
                const auto callFunc = Assets.Load<MaterialFunction>(Content::ResolveRuntimeObjectId((Guid)_callStack[i]->Values[0]));
...
void MaterialGenerator::ProcessGroupFunction(Box* box, Node* node, Value& value)
{
    ...
    case 1:
    {
        ...
        const auto function = Assets.Load<MaterialFunction>(Content::ResolveRuntimeObjectId((Guid)functionCallNode->Values[0]));
