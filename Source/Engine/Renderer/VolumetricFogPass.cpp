...
void VolumetricFogPass::Render(RenderContext& renderContext)
{
    ...
    if (EnumHasAnyFlags(renderContext.View.Flags, ViewFlags::GI))
    {
        ...
        case GlobalIlluminationMode::HDDAGI:
            // Volumetric fog uses low-frequency ambient/skylight when directional probe sampling is active
            break;
        }
    }
    ...
    // Light Scattering
    {
        ...
        if (useDDGI)
        {
            ...
            context->BindSR(7, bindingDataDDGI.ProbeStates);
            context->BindSR(8, bindingDataDDGI.ProbesDistance);
            context->BindSR(9, bindingDataDDGI.ProbesIrradiance);
            csIndex = 1;
        }
        else
        {
