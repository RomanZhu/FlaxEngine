...
LRESULT WindowsWindow::WndProc(UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_MOUSEMOVE:
    {
        ...
        if (_isTrackingMouse && _isUsingMouseOffset)
        {
            ...
            const float edgePadding = 3.0f;
            const Float2 wrapMin = desktopLocation + edgePadding;
            const Float2 wrapMax = desktopSize - edgePadding;
            const Float2 wrapSpan = wrapMax - wrapMin;

            // Use inverse edge mappings so crossing one edge and retracing across
            // the opposite edge cancels the tracking offset exactly. Ignore the
            // synthetic move generated by SetMousePosition to avoid flipping back.
            if (_isHorizontalFlippingMouse)
            {
                _isHorizontalFlippingMouse = false;
            }
            else if (mousePosition.X <= wrapMin.X)
            {
                newMousePosition.X += wrapSpan.X;
                _isHorizontalFlippingMouse = true;
            }
            else if (mousePosition.X >= wrapMax.X)
            {
                newMousePosition.X -= wrapSpan.X;
                _isHorizontalFlippingMouse = true;
            }
            ...
            else if (mousePosition.Y <= wrapMin.Y)
            {
                newMousePosition.Y += wrapSpan.Y;
                _isVerticalFlippingMouse = true;
            }
            else if (mousePosition.Y >= wrapMax.Y)
            {
                newMousePosition.Y -= wrapSpan.Y;
                _isVerticalFlippingMouse = true;
            }
            if (!Float2::NearEqual(mousePosition, newMousePosition))
            {
                ...
                // SetMousePosition updates the input cursor immediately. Do not
                // forward this pre-wrap WM_MOUSEMOVE afterwards or it will
                // overwrite the wrapped position with the stale edge position.
                // The synthetic move generated by SetMousePosition will deliver
                // the new physical position while the tracking offset preserves
                // the continuous virtual coordinate.
                return true;
            }
        }
