...
namespace FlaxEditor.Modules
{
    ...
    public sealed class UIModule : EditorModule
    {
        ...
        private void InitMainMenu(RootControl mainWindow)
        {
            ...
            _menuCreateParentForSelectedActors = cm.AddButton("Group", inputOptions.GroupSelectedActors, Editor.SceneEditing.CreateParentForSelectedActors);
            ...
            _menuToolsBuildAllMeshesSDF.LinkTooltip("Generates Sign Distance Field texture for all meshes and CSG geometry used in loaded scenes. Use with 'F' key pressed to force rebuild SDF for meshes with existing one.");
        ...
        private void OnMenuEditShowHide(Control control)
        {
            ...
            _menuCreateParentForSelectedActors.Enabled = canEditScene;
        ...
        {
            if (!FlaxEngine.Content.GetRuntimeAssetInfo(Level.Scenes[0].ID, out var sceneInfo) || sceneInfo.ObjectID == Guid.Empty)
            {
