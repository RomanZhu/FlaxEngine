...
- Use `flax assets batch --input <manifest.json>` for bulk imports, typed model/texture reimports, creation, material-instance configuration, refresh, and persistence checks. Add `--verify-reload` when the result must prove that changed assets reload from disk. Canonical texture reimports use `"importer": "texture"` with partial `importOptions`; unspecified tracked settings are preserved, metadata is applied atomically, and the exact texture build must finish before the operation succeeds.
...
- Use `flax visject groups list`, `visject asset inspect|validate`, `visject parameter add`, and the node/connect routes for material and Animation Graph assets. These are live Editor, graph-native operations and mutations persist through Flax's Visject serializer.
