...
- Use `flax player status|pause|resume|step|quit` and `flax player input key|pointer|reset` (or the `flax runtime input ...` alias) for a development Editor's embedded Player or a registered Development Player. Input is virtual Flax device input, not OS hardware; gamepad/action synthesis is intentionally unsupported.
